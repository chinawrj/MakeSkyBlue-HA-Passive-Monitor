...
> 适用范围：另一套、另行授权的测试环境。2026-09-07 本轮仅整理文档，当前设备
> 保持 monitor，不刷写、不重启、不拔插、不切换 preview，也不修改 HA 自动化。
> 下文烧录、热插拔和重建抓包窗口步骤不是当前设备的待执行事项。
>
> 已知阻断：alpha.10 的 HA 事件变量存在未求值字符串问题，实际 File CSV 尚未
> 正常积累；详见[持久化链路诊断](atoms3-g1-g2-modbus-monitor.md#known-capture-blocker-2026-09-07)。
> 现版本可以复现被动解析，不能据此承诺 Step 9/13 的持久化验收会通过。
> 后续修复需单独发布与实测；本次不修改固件来消除这一阻断。
...
该 tag 是已发布的历史固件候选，不包含本次文档更新；文档以当前 main 为准，
复现实验必须另外记录所用文档 commit。不要从未知 fork 获取带凭据的预编译 firmware。
...
前置门槛：先在独立测试环境消除已知事件求值和 File notifier 问题，验证 payload
真实值、CSV 增长、ACK 增长与零覆盖；不能靠重新刷写当前 monitor 或反复重启
绕过门槛。下面只是未来合格采集窗口的流程，并非已经完成的验收记录。
